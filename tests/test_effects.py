...
import numpy as np
import pool, pool_weave, pipeline, effects
...
one = [sel[0]]
s1 = frames_of(effects.superimpose.generate(one, cache, layers=8, spread=0.2, fps=24, num_frames=8))
assert s1[0].shape == (360,640,3) and s1[0].dtype == np.uint8
s2 = frames_of(effects.superimpose.generate(sel, cache, layers=len(sel), spread=0.0, fps=24, num_frames=8))
assert not np.array_equal(s1[0], s2[0]), "spread/selection made no difference"
print(f"superimpose: 1-clip stack (8 layers, 0.2s) and {len(sel)}-clip instant both render ✓")

# blends
for b in effects.superimpose.BLENDS:
    f = frames_of(effects.superimpose.generate(sel, cache, layers=3, blend=b, fps=24, num_frames=2))
    assert f[0].dtype == np.uint8
print("blends:", ", ".join(effects.superimpose.BLENDS), "✓")

# ── parallax modes ────────────────────────────────────────────────────────────
for mode in ("none", "affine", "flow"):
    f = frames_of(effects.superimpose.generate(one, cache, layers=4, spread=0.1,
            parallax=mode, amount=0.6, fps=24, num_frames=3))
    assert f[0].dtype == np.uint8
...
for kind in effects.blur.KINDS:
    src = effects.passthrough.generate(one, cache, num_frames=6)
...
for kind in ("edge_bilateral", "anisotropic"):
    src = effects.passthrough.generate(one, cache, num_frames=3)
